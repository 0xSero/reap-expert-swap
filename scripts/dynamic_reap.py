    ...
    *,
    ...
    candidate_targets = {
        _normalize_layer_key(layer_key): max(
            ...
            int(math.ceil(specialist_targets[_normalize_layer_key(layer_key)] * candidate_pool_multiplier)),
        )
        for layer_key in layer_keys
    }
...
def validate_active_set_payload(payload: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    ...
    for layer_key, layer in per_layer.items():
        ...
        for slice_id in selected_slice_ids.get(layer_key, []):
        ...
        union_validation[layer_key] = {
            ...
            "selectedSliceIds": list(selected_slice_ids.get(layer_key, [])),
