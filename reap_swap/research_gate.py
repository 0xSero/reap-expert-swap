...
def _snapshot_metrics(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "dynamic_signature_transition_count": overall.get("dynamic_signature_transition_count"),
...
def _plan_budget_bytes(identity: dict[str, Any] | None) -> int | None:
    if not isinstance(identity, dict):
        return None
    value = identity.get("plan_budget_bytes")
    if value is None:
        value = identity.get("swappable_expert_budget_bytes")
    if value is None:
        return None
    try:
        return int(value)
...
def _plan_fingerprint(identity: dict[str, Any] | None) -> str | None:
    if not isinstance(identity, dict):
        return None
    value = identity.get("plan_sha256")
    if value is None:
        return None
    return str(value)


def _plan_mode(identity: dict[str, Any] | None) -> str | None:
    if not isinstance(identity, dict):
        return None
    value = identity.get("plan_mode")
    if value is None:
        return None
    return str(value)


def _plan_paths_equivalent(
    expected_identity: dict[str, Any],
    observed_identity: dict[str, Any],
) -> bool:
    expected_path = expected_identity.get("plan_path")
    observed_path = observed_identity.get("plan_path")
    if not expected_path or not observed_path:
        return True
    if str(expected_path) == str(observed_path):
        return True

    expected_fp = _plan_fingerprint(expected_identity)
    observed_fp = _plan_fingerprint(observed_identity)
    if expected_fp and observed_fp:
        return expected_fp == observed_fp

    expected_mode = _plan_mode(expected_identity)
    observed_mode = _plan_mode(observed_identity)
    expected_budget = _plan_budget_bytes(expected_identity)
    observed_budget = _plan_budget_bytes(observed_identity)
    if (
        expected_mode
        and observed_mode
        and expected_mode == observed_mode
        and expected_budget is not None
        and observed_budget is not None
        and expected_budget == observed_budget
    ):
        # Local controller path and remote runtime path can differ while still
        # representing the same plan artifact. Accept when mode+budget match.
        return True
    return False
...
def evaluate_payload_gate(
    ...
    *,
    ...
    # Check retained_metrics_status: if the baseline comparison is flagged as
    # invalid (e.g., mismatched seed/sample_count), the numeric retained metrics
    # cannot be trusted for gating decisions.
    retained_status = comparison.get("retained_metrics_status")
    if retained_status == "invalid_unmatched_baseline":
        match_reasons = comparison.get("baseline_match_reasons") or []
        reason_detail = f" (mismatched: {', '.join(match_reasons)})" if match_reasons else ""
        structural_errors.append(
            f"baseline comparison is invalid_unmatched_baseline{reason_detail}"
        )
    ...
    if payload.get("mode") == "dynamic":
        ...
        if not isinstance(plan_budget, dict):
            ...
            if not isinstance(readiness_evidence, dict):
                ...
                readiness_identity_payload = readiness_evidence.get("identity")
                if not isinstance(readiness_identity_payload, dict):
                    structural_errors.append("runtime readiness evidence missing identity payload")
                else:
                    readiness_host = readiness_identity_payload.get("host")
                    readiness_port = readiness_identity_payload.get("port")
                    readiness_plan_file = readiness_identity_payload.get("plan_file")
                    ...
                    readiness_identity = {
                        "plan_path": readiness_plan_file,
                        "plan_mode": readiness_identity_payload.get("plan_mode"),
                        "plan_budget_bytes": readiness_identity_payload.get(
                            "plan_budget_bytes"
                        ),
                    }
                    if isinstance(plan_identity, dict) and not _plan_paths_equivalent(
                        plan_identity, readiness_identity
                    ):
                        structural_errors.append(
            ...
            if (
                isinstance(plan_identity, dict)
                and isinstance(runtime_identity, dict)
                and runtime_plan_path
                and not _plan_paths_equivalent(plan_identity, runtime_identity)
            ):
                structural_errors.append(
        ...
        for row in payload.get("results") or []:
            ...
            if not isinstance(swap_plan_identity, dict):
                ...
                if not _plan_paths_equivalent(plan_identity, swap_plan_identity):
                    request_linkage_errors.append(
    ...
    check_max(
        "avg_swap_time_s",
        (
            overall.get("avg_warm_swap_time_s")
            if overall.get("warm_swap_count")
            else overall.get("avg_swap_time_s")
        ),
