...
def _coalesce_config_value(config: dict[str, Any], *keys: str, default: Any = None) -> Any:
    for key in keys:
...
def normalize_moe_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if model_type == "deepseek_v3":
        normalized["num_experts"] = _coalesce_config_value(
            normalized,
            "num_experts",
            "n_routed_experts",
        )
        normalized["shared_expert_intermediate_size"] = _coalesce_config_value(
            normalized,
            "shared_expert_intermediate_size",
            default=int(normalized.get("moe_intermediate_size", 0)) * int(normalized.get("n_shared_experts", 0) or 0),
        )
    return normalized
...
def estimate_deepseek_v3_bf16_bytes(config: dict[str, Any]) -> dict[str, Any]:
    config = normalize_moe_config(config)
    hidden_size = int(config["hidden_size"])
    num_hidden_layers = int(config["num_hidden_layers"])
    num_experts = int(config["num_experts"])
    moe_intermediate_size = int(config["moe_intermediate_size"])
    shared_expert_intermediate_size = int(config.get("shared_expert_intermediate_size") or 0)
    vocab_size = int(config["vocab_size"])
    tie_word_embeddings = bool(config.get("tie_word_embeddings", False))

    q_lora_rank = int(config.get("q_lora_rank") or 0)
    kv_lora_rank = int(config.get("kv_lora_rank") or 0)
    qk_nope_head_dim = int(config.get("qk_nope_head_dim") or 0)
    qk_rope_head_dim = int(config.get("qk_rope_head_dim") or 0)
    v_head_dim = int(config.get("v_head_dim") or 0)
    num_attention_heads = int(config.get("num_attention_heads") or 0)
    num_key_value_heads = int(config.get("num_key_value_heads") or num_attention_heads or 0)
    ...
    if q_lora_rank and kv_lora_rank and qk_nope_head_dim and qk_rope_head_dim and v_head_dim and num_attention_heads:
        q_head_dim = qk_nope_head_dim + qk_rope_head_dim
        q_proj = hidden_size * q_lora_rank + q_lora_rank * (num_attention_heads * q_head_dim)
        kv_proj = hidden_size * kv_lora_rank + kv_lora_rank * (num_key_value_heads * (qk_nope_head_dim + v_head_dim))
        o_proj = (num_attention_heads * v_head_dim) * hidden_size
        attention_params_per_layer = q_proj + kv_proj + o_proj
    else:
        attention_params_per_layer = 4 * hidden_size * hidden_size

    attention_params = num_hidden_layers * attention_params_per_layer
    router_params = num_hidden_layers * (hidden_size * num_experts)
    embedding_params = vocab_size * hidden_size
    lm_head_params = 0 if tie_word_embeddings else vocab_size * hidden_size
    norm_and_misc_params = num_hidden_layers * (4 * hidden_size) + hidden_size
...
def estimate_resident_bf16(record: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if model_type == "qwen2_moe":
        ...
        model_bytes = estimate_deepseek_v3_bf16_bytes(config)
        method = "model_estimate_deepseek_v3"
    else:
        return None
