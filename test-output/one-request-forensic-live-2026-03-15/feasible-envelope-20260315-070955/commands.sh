...
RUN_DIR="/Users/sero/ai/autoresearch/test-output/one-request-forensic-live-2026-03-15/feasible-envelope-20260315-070955"
...
printf '%s\n' "run_dir=$RUN_DIR" "plan_local=$PLAN_LOCAL" "baseline_json=$BASELINE_JSON" "remote_repo=$REMOTE_REPO" "remote_port=$REMOTE_PORT" "local_port=$LOCAL_PORT" > "$RUN_DIR/context.txt"
...
# Ensure chosen port is free; no broad kill.
ssh "$REMOTE" "bash -lc 'pgrep -af \"vllm_multiplex_server.py.*--port $REMOTE_PORT\" || true'" | tee "$RUN_DIR/preexisting-port-check.txt"
...
# Bounded replay attempt proceeds only when ready.
if [ "$ready" -eq 1 ]; then
...
crash=(forensic.get('classification') or {}).get('crash') if forensic else 'missing'
...
failure_mode = 'none' if success else (
  ...
  'dynamic_missing' if not (run/'dynamic.json').exists() else
...
proof=['# One-request forensic replay proof','','## Envelope','- `CUDA_VISIBLE_DEVICES=2,3`','- `tensor-parallel-size=2`','- `max-model-len=3072`','- `max-num-seqs=1`','- `reasoning-parser=qwen3`','- `cpu-offload-gb=28`','- `swap-space=32`','- `gpu-memory-utilization=0.90`','','## Attempt policy','- single bounded attempt (no relaunch retries)','','## Required artifacts']
...
proof += ['', '## Outcome', f"- success: `{str(success).lower()}`", f"- failure_mode: `{failure_mode}`", f"- collector_rc: `{collector_rc}`"]
